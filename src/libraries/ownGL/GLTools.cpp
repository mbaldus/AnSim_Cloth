...
bool GLTools::isRetinaDisplay(GLFWwindow* window)
{
	int width, height;
	glfwGetWindowSize(window, &width, &height);
	if(GLTools::getWidth(window) <= width)
		return false;

	return true;
}

float GLTools::getRetinaDisplayScale(GLFWwindow* window)
{
	int width, height;
	glfwGetWindowSize(window, &width, &height);
	if(GLTools::getWidth(window) <= width)
	...
	return GLTools::getWidth(window) / width;
}
