...
void ImDrawList::UpdateClipRect() {
    ...
    if (curr_cmd->ElemCount == 0 && prev_cmd && memcmp(&prev_cmd->ClipRect, &curr_clip_rect, sizeof(ImVec4)) == 0 &&
        prev_cmd->TextureId == GetCurrentTextureId() && prev_cmd->UserCallback ==
                                                        nullptr)
        CmdBuffer.pop_back();
    else
...
void ImDrawList::UpdateTextureID() {
    // If current command is used with different settings we need to add a new command
    const auto curr_texture_id = GetCurrentTextureId();
    ...
    if (prev_cmd && prev_cmd->TextureId == curr_texture_id &&
        memcmp(&prev_cmd->ClipRect, &GetCurrentClipRect(), sizeof(ImVec4)) == 0 && prev_cmd->UserCallback ==
                                                                                   nullptr)
        CmdBuffer.pop_back();
    else
...
void ImDrawList::AddPolyline(const ImVec2 *points, const int points_count, ImU32 col, bool closed, float thickness,
    ...
    if (anti_aliased) {
        ...
        // Temporary buffer
        auto *temp_normals = (ImVec2 *) alloca(points_count * (thick_line ? 5 : 3) * sizeof(ImVec2));
...
void ImDrawList::AddConvexPolyFilled(const ImVec2 *points, const int points_count, ImU32 col, bool anti_aliased) {
    ...
    if (anti_aliased) {
        ...
        // Compute normals
        auto *temp_normals = (ImVec2 *) alloca(points_count * sizeof(ImVec2));
...
ImFont *ImFontAtlas::AddFont(const ImFontConfig *font_cfg) {
    ...
    if (!font_cfg->MergeMode) {
        auto *font = (ImFont *) ImGui::MemAlloc(sizeof(ImFont));
        IM_PLACEMENT_NEW(font) ImFont();
        Fonts.push_back(font);
    } else {
        IM_ASSERT(
                !Fonts.empty()); // When using MergeMode make sure that a font has already been added before. You can use ImGui::AddFontDefault() to add the default imgui font.
    }
    ...
    if (!new_font_cfg.FontDataOwnedByAtlas) {
        new_font_cfg.FontData = ImGui::MemAlloc(static_cast<size_t>(new_font_cfg.FontDataSize));
...
static unsigned int Decode85Byte(char c) { return static_cast<unsigned int>(c >= '\\' ? c - 36 : c - 35); }
...
ImFont *ImFontAtlas::AddFontFromFileTTF(const char *filename, float size_pixels, const ImFontConfig *font_cfg_template,
    ...
    if (!data) {
        IM_ASSERT(false); // Could not load file.
        return nullptr;
    }
    ImFontConfig font_cfg = font_cfg_template ? *font_cfg_template : ImFontConfig();
...
ImFontAtlas::AddFontFromMemoryCompressedTTF(const void *compressed_ttf_data, int compressed_ttf_size, float size_pixels,
    ...
    auto buf_decompressed_data = (unsigned char *) ImGui::MemAlloc(buf_decompressed_size);
